...
namespace Config
{
	...
	void Manager::SaveToDisk()
	{
		...
		outFile << "; This file was generated by " << g_pluginName << ".\n";
		outFile << "; Last saved: " << std::put_time(&timeInfo, "%Y-%m-%d %H:%M:%S") << "\n\n";

		bool wroteAny = false;
		const auto writeSection = [&](const std::string& section) {
			...
			for (const auto& [key, setting] : it->second) {
		...
		writeSection("");
		writeSection("Input");
		writeSection("Haptics");

		for (const auto& [section, _] : sections) {
			if (section.empty() || section == "Input" || section == "Haptics") {
				continue;
			}
